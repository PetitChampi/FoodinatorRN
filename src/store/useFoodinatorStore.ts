...
import type { FoodinatorStore, FoodinatorState, MealSlot } from "./types";
import { DEFAULT_RESTOCK_CATEGORIES } from "./constants";
...
const foodinatorStoreCreator: StateCreator<FoodinatorStore> = (set, get) => {
  return {
    ...
    addRestockItem: (name, categoryEmoji) => set(produce((state: FoodinatorState) => {
      const id = `restock_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    toggleRestockItem: (id) => set(produce((state: FoodinatorState) => {
      const restockItem = state.restockList.find(item => item.id === id);
      if (restockItem) {
        restockItem.checked = !restockItem.checked;
      }
    })),
      ...
      if (state && state.restockCategories) {
        ...
        const mergedCategories = DEFAULT_RESTOCK_CATEGORIES.map(defaultCat => {
          ...
          if (existingCat) {
            ...
            defaultCat.items.forEach((defaultItem: string) => {
